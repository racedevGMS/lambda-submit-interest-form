...
def _verify_recaptcha(token: str) -> bool:
    print(f"reCAPTCHA: verifying token (len={len(token) if token else 0})")
    print(f"reCAPTCHA: secret key set = {bool(_RECAPTCHA_SECRET)}")
    ...
    passed = result.get("success") and result.get("score", 0) >= _SCORE_THRESHOLD
    print(f"reCAPTCHA: success={result.get('success')} score={result.get('score')} threshold={_SCORE_THRESHOLD} passed={passed}")
    return passed
...
def InterestFormSubmissionHandler(body):
    try:
        ...
        if not token or not _verify_recaptcha(token):
            print("reCAPTCHA: verification failed — returning 400")
            return {
                "statusCode": 400,
